...
| [upgrades.md](upgrades.md) | Planned upgrades (not built): card categories as ENS records, a card-shop client | Same as above | 2026-09-26 15:00, **after kickoff** |
...
- **Planned upgrades** (not built yet): U1 records each card's category as ENS text records (`card.category`, `card.game` or `card.sport`, `card.subject` and so on, following PSA's cert fields). It needs a TitleControllerV3 with a fallback to the old controllers, and swapping the resolver before the final lock. U2 adds a card-shop client: a counter mode for buying in and verifying, inventory, and a storefront.
