...
- [x] P5-8 部署到公開網址：https://nafuda.sololin.xyz 。**改由 [web-v2-plan.md](web-v2-plan.md) 處理**（22:03 作者決定自架在 Mac mini，用 Docker 加 Cloudflare Tunnel；Cloudflare Workers 的靜態部署保留當熱備援）。GitHub Actions 的 `web.yml` 只負責測試和 build
