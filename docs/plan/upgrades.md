...
> **狀態（2026-09-26 16:30 JST）：作者選了輕量版，已經上線。**
> - **不改合約。** 用 BGS-Sim V2 剩下的 4 個欄位存 `card.category`、`card.game` 或 `card.sport`、`card.year`、`card.language`。
> - **已經發了 8 張有分類的示範權狀**（1004827401–408）：
>   - Pokémon ×2、遊戲王 ×2、One Piece、Magic
>   - 棒球、籃球
> - **評級商後台**：Issue 頁加了分類選單。
> - **收藏家 app**：權狀頁、slab 標籤和 Explore 篩選都會顯示分類。
> - **還沒做的部分**：下面的完整版（V3，三家評級商都有分類）仍然只是規劃。
