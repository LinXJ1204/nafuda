...
import { SimulatedChip } from './chips.ts'
...
const genuine = new SimulatedChip('psa-sim', 'genuine', '12345678')
const clone = new SimulatedChip('psa-sim', 'clone', '12345678')
...
test('simulated chips derive the same keys as demo/slabs.json', async () => {
  assert.equal(genuine.address, privateKeyToAccount(slab.genuine.privateKey).address)
  assert.equal(clone.address, privateKeyToAccount(slab.clone.privateKey).address)
})
